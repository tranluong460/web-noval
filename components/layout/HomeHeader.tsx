    ...
    >
      ...
      <div className="border-b border-emerald-100/50 dark:border-slate-700/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            {/* Logo Section - Phong cách cổ điển Trung Hoa */}
            <Link 
              href="/" 
            ...
            >
              ...
              {/* Tên website với font serif cổ điển */}
              <div className="hidden sm:block">
                <h1 className="text-2xl lg:text-3xl font-serif font-bold bg-gradient-to-r from-emerald-600 via-cyan-600 to-teal-600 bg-clip-text text-transparent group-hover:from-emerald-500 group-hover:via-cyan-500 group-hover:to-teal-500 transition-all duration-300">
                  Tu Tiên Các
                </h1>
            ...
            <div className="flex-1 max-w-2xl mx-4 lg:mx-8">
              <form onSubmit={handleSearch} className="relative group">
                <div className={clsx(
                  ...
                  <div className={clsx(
                    'absolute inset-0 bg-gradient-to-r from-emerald-400/20 via-cyan-400/20 to-teal-400/20 transition-opacity duration-300',
                  ...
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-3 bg-transparent text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none text-sm lg:text-base"
                      aria-label="Tìm kiếm truyện"
                    />
                    
                    {/* Keyboard shortcut hint */}
                    <div className="hidden lg:flex absolute right-3 items-center space-x-1 text-xs text-gray-400 dark:text-gray-500">
              ...
              {user ? (
                ...
                <div className="relative" ref={userDropdownRef}>
                  <button
                    ...
                    className="flex items-center space-x-3 p-2 rounded-full bg-white/50 dark:bg-slate-800/50 hover:bg-white/80 dark:hover:bg-slate-700/80 transition-all duration-200 group"
                  ...
                  {isUserDropdownOpen && (
                    <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-emerald-200/50 dark:border-slate-600/50 py-2 animate-dropdown-fade-in z-50">
                      {/* User info header */}
              ...
              <div key={index} className="relative group">
                <Link
                  ...
                  className={clsx(
                    ...
                    'text-gray-700 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-cyan-400',
                ...
                {item.children && (
                  <div className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-emerald-200/50 dark:border-slate-600/50 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-40">
                    {item.children.map((child, childIndex) => (
                      <Link
                        key={childIndex}
                        href={child.href}
                        className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-cyan-400 hover:bg-emerald-50 dark:hover:bg-slate-700 transition-colors duration-200"
                      >
                        {child.label}
                      </Link>
                    ))}
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-black/50 backdrop-blur-sm" onClick={() => setIsMobileMenuOpen(false)}>
          <div 
            className="absolute right-0 top-0 h-full w-80 max-w-[90vw] bg-white dark:bg-slate-900 shadow-xl transform transition-transform duration-300 ease-out"
              ...
              {mainMenuItems.map((item, index) => (
                <div key={index}>
                  <Link
                    href={item.href}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center space-x-3 p-3 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-slate-800 transition-colors duration-200"
                  >
                    {item.icon}
                    <span className="font-medium">{item.label}</span>
                    {item.isHot && (
                      <span className="px-2 py-0.5 text-xs bg-red-500 text-white rounded-full">HOT</span>
                    )}
                    {item.isNew && (
                    ...
                    <div className="ml-6 mt-2 space-y-1">
                      {item.children.map((child, childIndex) => (
                        <Link
                          ...
                          href={child.href}
                          onClick={() => setIsMobileMenuOpen(false)}
                          className="block p-2 text-sm text-gray-600 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-cyan-400 transition-colors duration-200"
                        >
                          {child.label}
                        </Link>
                      ))}
